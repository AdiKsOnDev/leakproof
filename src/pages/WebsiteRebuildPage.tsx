  ...
  return (
    <div className="rebuild-page">
      ...
      <section className="section rebuild-comparison" id="rebuild-comparison" aria-labelledby="rebuild-comparison-title">
        <div className="container">
          <h2 id="rebuild-comparison-title">Same business. Less to get in the way.</h2>
          <p>A fictional salon, before and after a rebuild. The services haven’t changed. How you find them has.</p>
      ...
      <section className="section">
        <div className="container rebuild-included">
          <div><h2>More than a new look.</h2><p>We work on the content, structure and booking steps as well as the design.</p></div>
          <div>{deliverables.map(([title, description]) => <article key={title}><Check size={20} aria-hidden="true" /><div><h3>{title}</h3><p>{description}</p></div></article>)}</div>
        </div>
      </section>

      <section className="section rebuild-ownership">
        <div className="container"><h2>It’s your website.<br />You decide who maintains it.</h2><p>We hand over the source code, repository, access and documentation. Keep working with us or take it to another developer.</p><Link className="arrow-link" to="/services">What’s included in our work <ArrowLeft size={17} aria-hidden="true" /></Link></div>
      </section>
